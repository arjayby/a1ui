...
[Multichain Swap](/components/multichain-swap) uses this component for its networks, tokens, and slippage values. Token names and balances appear in its option rows; filtering and quote validation remain in the swap component. Installing Multichain Swap includes the same Select Menu source file. Select Menu installs with `@base-ui/react`, `clsx`, and `lucide-react`.
