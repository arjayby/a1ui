...
Text fills the available spaces on either side and between the arms of the X. Gaps narrower than two characters are skipped. Drag with a mouse, pen, or touch. Focus the X and use arrow keys to move it by 8 pixels, or hold Shift for 32-pixel steps. Home centers the X in the full layout. The X stays inside the drag area when the container resizes.
