...
export function ArticlePage() {
  return (
    <>
      <SectionRail sections={sections} gap={0} className="fixed top-1/2 left-4 -translate-y-1/2" />
...
The active link receives `aria-current="location"`. Every link also exposes `data-state="pending"`, `active`, or `complete` for styling. Hovering or focusing a link expands the nearby markers into a tapered group.
